...
async function handleAuth({event, resolve}) {
	...
	const noAuth = ["/", "/api/otp-auth", "/verify-email", "/api/verify-otp"];

	
	if (noAuth.includes(url.pathname)) {
		return resolve(event);
	}
	...
	if (new Date(customer.expiresAt).getTime() < Date.now()) {
		await db.delete(sessions).where(eq(sessions.id, customer.sessionId));
		cookies.delete("better-auth.session_token");
		throw redirect(302, "/");
	}
	
	event.locals.userId = customer.customerId;
